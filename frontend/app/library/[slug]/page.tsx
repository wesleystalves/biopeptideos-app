        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <div className="bg-[#0f172a] border border-white/10 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
                ...
                <p className="text-sm text-slate-300 leading-relaxed">
                    Esta plataforma é exclusivamente para <strong className="text-white">fins educacionais e informativos</strong>. Todo o conteúdo apresentado é baseado em estudos científicos publicados e tem caráter de referência acadêmica.
                </p>
...
export default function PeptideDetailPage() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'protocol' | 'science' | 'stacks'>('overview');
                ...
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <div className="glass-card overflow-hidden">
                        <div className="grid md:grid-cols-[320px_1fr]">
                            ...
                            <div className="relative bg-gradient-to-br from-brand-900/40 to-black h-56 md:h-auto">
                                {peptide.imageUrl ? (
                                    <img src={peptide.imageUrl} alt={peptide.name} className="w-full h-full object-cover" />
                                ) : (
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                {/* Quick stats from detail */}
                                {hasDetail && (
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                                            <div className="text-xs text-slate-500 mb-1">Classificação</div>
                                            <div className="text-xs text-white font-medium leading-tight">{detail.classification}</div>
                                        </div>
                                        <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                                            <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-1">
                                                <Clock className="w-3 h-3" /> Meia-Vida
                                            </div>
                                            <div className="text-xs text-white font-medium">{detail.halfLife}</div>
                                        </div>
                                    </div>
                                )}

                                {/* Evidence level + difficulty */}
                                {hasDetail && (
                                    <div className="flex flex-wrap items-center gap-2">
                                        <EvidenceBadge level={detail.evidenceLevel} />
                                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${detail.reconstitutionDifficulty === 'Fácil' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                                            Reconstituição: {detail.reconstitutionDifficulty}
                                        </span>
                                    </div>
                                )}
                                ...
                                {/* Alternative names */}
                                {hasDetail && detail.alternativeNames.length > 0 && (
                                    <div className="text-xs text-slate-500">
                                        Também conhecido como: {detail.alternativeNames.join(', ')}
                                    </div>
                                )}
                    ...
                    {(!hasDetail || activeTab === 'overview') && (
                        <div className="space-y-5">
                            {/* What Is */}
                            {hasDetail && detail.whatIs && (
                                <div className="glass-card p-6">
                                    <h2 className="text-lg font-bold text-white mb-3 flex items-center gap-2">
                                        <BookOpen className="w-5 h-5 text-brand-400" /> O Que É
                                    </h2>
                                    <p className="text-slate-300 leading-relaxed text-sm">{detail.whatIs}</p>
                                </div>
                            )}
                            ...
                            <div className="glass-card p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {(hasDetail ? detail.benefits : peptide.benefits).map(b => (
                                        <div key={b} className="flex items-start gap-3 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/15">
                            ...
                            {/* Timeline */}
                            {hasDetail && detail.timeline && detail.timeline.length > 0 && (
                                <div className="glass-card p-6">
                    ...
                    {/* ── TAB: PROTOCOLO ── */}
                    {hasDetail && activeTab === 'protocol' && (
                        <div className="space-y-5">
                            {/* Protocol */}
                            {detail.protocol && (
                                <div className="glass-card p-6">
                                    <h2 className="text-lg font-bold text-white mb-1">{detail.protocol.title}</h2>
                                    <p className="text-sm text-brand-400 mb-4">{detail.protocol.route}</p>
                                    <div className="space-y-3">
                                        {detail.protocol.phases.map((phase, i) => (
                            ...
                            {detail.dosageByIndication && detail.dosageByIndication.length > 0 && (
                                <div className="glass-card overflow-hidden">
                                    <div className="p-5 border-b border-white/5">
                                        <h2 className="font-bold text-white">📊 Dosagem por Indicação</h2>
                                    </div>
                                    <div className="divide-y divide-white/5">
                                        {detail.dosageByIndication.map((d, i) => (
                                            <div key={i} className="p-4 hover:bg-white/3 transition-colors">
                                                <div className="font-medium text-white text-sm mb-2">{d.indication}</div>
                                                <div className="grid grid-cols-3 gap-3">
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Dose</div>
                                                        <div className="text-sm text-brand-400 font-medium">{d.dose}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Frequência</div>
                                                        <div className="text-xs text-slate-300">{d.frequency}</div>
                                                    </div>
                                                    <div>
                                                        <div className="text-[10px] text-slate-500 uppercase">Duração</div>
                                                        <div className="text-xs text-slate-300">{d.duration}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {/* Reconstitution */}
                            {detail.reconstitution && detail.reconstitution.length > 0 && (
                                <div className="glass-card p-6">
                                    <h2 className="font-bold text-white mb-4">💧 Reconstituição</h2>
                                    <ol className="space-y-2">
                                        {detail.reconstitution.map((step, i) => (
                    ...
                    {/* ── TAB: MECANISMO ── */}
                    {hasDetail && activeTab === 'science' && (
                        <div className="space-y-5">
                            {/* Mechanism steps */}
                            {detail.mechanism && detail.mechanism.length > 0 && (
                                <div className="glass-card p-6">
                                    <h2 className="text-lg font-bold text-white mb-4">⚙️ Mecanismo de Ação</h2>
                                    <div className="space-y-3">
                                        {detail.mechanism.map((step, i) => (
                                            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/3 border border-white/8">
                                                <span className="w-6 h-6 rounded-full bg-violet-600/20 border border-violet-500/25 text-violet-400 text-xs font-bold flex items-center justify-center flex-shrink-0">{i + 1}</span>
                            ...
                            {/* Mechanism detailed */}
                            {detail.mechanismDetailed && (
                                <div className="glass-card p-6">
                                    <h2 className="font-bold text-white mb-3">📖 Explicação Detalhada</h2>
                                    <p className="text-slate-300 text-sm leading-relaxed">{detail.mechanismDetailed}</p>
                                </div>
                            )}
                    ...
                    {hasDetail && activeTab === 'stacks' && (
                        <div className="space-y-5">
                                            ...
                                            <div key={i} className="flex items-start gap-3 p-4">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-2 mb-0.5">
                                                        ...
                                                        <span className={`text-[10px] px-2 py-0.5 rounded-full border ${inter.type === 'synergic' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                                                                inter.type === 'compatible' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                                                                    inter.type === 'monitor' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                            ...
                            {/* If no stacks detail */}
                            {(!detail.stacks || detail.stacks.length === 0) && (!detail.interactions || detail.interactions.length === 0) && (
                                <div className="glass-card p-12 text-center">
                                    <Zap className="w-10 h-10 text-slate-700 mx-auto mb-3" />
                                    <p className="text-slate-400">Explore a Biblioteca de Stacks para combinações com este peptídeo.</p>
                                    <Link href="/stacks" className="btn-primary inline-flex items-center gap-2 mt-4">
                                        Ver Stacks <ChevronRight className="w-4 h-4" />
                                    </Link>
                                </div>
                            )}
                        </div>
